####################################################################
##  lehdr_California_2022.r
##
##  Test for new LODES data released today, 11/19/2024 for 2022
##
##    lehdr version 1.1.3, updated 11/19/2024 to include LODES version 8.3
##  Explore the Residence Area Characteristics (RAC) and
##             Work Area Characteristics (WAC) files
##
##  California, statewide, various geographic levels, 2022
...
rac1 <- lehdr::grab_lodes(state = 'ca', year = 2022, lodes_type = "rac", 
...
start_timer <- Sys.time() # start stopwatch on this function
wac1 <- lehdr::grab_lodes(state = 'ca', year = 2022, lodes_type = "wac", 
                          use_cache=TRUE, version="LODES8",
                          job_type = 'JT00',   # All Jobs
...
start_timer <- Sys.time() # start stopwatch on this function
od1 <- lehdr::grab_lodes(state = 'ca', year = 2022, lodes_type = "od", 
                          use_cache=TRUE, version="LODES8",
                          job_type = 'JT00',   # All Jobs
...
start_timer <- Sys.time() # start stopwatch on this function
rac2 <- lehdr::grab_lodes(state = 'ca', year = 2022, lodes_type = "rac", 
                          use_cache=TRUE, version="LODES8",
                          job_type = 'JT00',   # All Jobs
...
start_timer <- Sys.time() # start stopwatch on this function
wac2 <- lehdr::grab_lodes(state = 'ca', year = 2022, lodes_type = "wac", 
                          use_cache=TRUE, version="LODES8",
                          job_type = 'JT00',   # All Jobs
...
start_timer <- Sys.time() # start stopwatch on this function
rac3 <- lehdr::grab_lodes(state = 'ca', year = 2022, lodes_type = "rac", 
                          use_cache=TRUE, version="LODES8",
                          job_type = 'JT00',   # All Jobs
...
start_timer <- Sys.time() # start stopwatch on this function
wac3 <- lehdr::grab_lodes(state = 'ca', year = 2022, lodes_type = "wac", 
                          use_cache=TRUE, version="LODES8",
                          job_type = 'JT00',   # All Jobs
...
start_timer <- Sys.time() # start stopwatch on this function
rac_state <- lehdr::grab_lodes(state = 'ca', year = 2022, lodes_type = "rac", 
                               use_cache=TRUE, version="LODES8",
                               job_type = 'JT00',   # All Jobs
...
start_timer <- Sys.time() # start stopwatch on this function
wac_state <- lehdr::grab_lodes(state = 'ca', year = 2022, lodes_type = "wac", 
                               use_cache=TRUE, version="LODES8",
                               job_type = 'JT00',   # All Jobs
...
CA_blkgrp_work_2022 <- dplyr::left_join(geo_BlkGrps1,wac3,by="w_bg") %>%
...
write.csv(summary1,   "lehd_California_BlkGrp_JobDensity_Groups_2022.csv")
write.csv(summary2a,  "lehd_California_BlkGrp_JobDensity_nblkgrps_county_2022.csv")
write.csv(summary2b,  "lehd_California_BlkGrp_JobDensity_totjobs_county_2022.csv")
write.csv(summary2c,  "lehd_California_BlkGrp_JobDensity_landarea_county_2022.csv")
...
plot(CA_blkgrp_work_2022["density_grp"])

mapview(CA_blkgrp_work_2022, zcol="density_grp")

###########################################################################
#  Select SF Bay Area block groups for mapping.....
bayarea_bg_work_2022 <- CA_blkgrp_work_2022 %>% 
    dplyr::filter(COUNTYFP %in% c("001","013","041","055","075","081","085","095","097"))

suggest_crs(bayarea_bg_work_2022) # get a listing of suggested coord refs for this area
suggest_top_crs(bayarea_bg_work_2022) # top suggested for this area

bayarea_bg_work_2022_p <- st_transform(bayarea_bg_work_2022,26942) # BLM11N, NAD83

ggplot() + 
  geom_sf(data=bayarea_bg_work_2022_p, alpha=1.0, aes(fill=density_grp), color=NA) + 
  scale_fill_manual(values=c("seagreen", "seagreen1","yellow","tan1","red","purple")) +
  theme_void() + 
  theme(plot.title    = element_text(hjust = 0.5)) +
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  labs(title = "Density of Workers-at-Work, San Francisco Bay Area: 2022",
     subtitle = "Census Bureau LEHD/LODES estimates for 2022",
     caption = "Created: CPurvis; Data sources: LEHD/LODES 8.3 (via lehdr)",
     fill = "Workers-at-Work Density Level")

setwd("~/Desktop/LEHD Research")

ggsave("sfbayarea_workers_at_work_density_2022.png", plot=last_plot(),
       dpi=600,width=8,height=7) 
###################################################################################
#  Select San Francisco County block groups for mapping.....
county75_bg_work_2022 <- CA_blkgrp_work_2022 %>% 
  dplyr::filter(COUNTYFP %in% c("075")) 
#   st_transform(26942)

ggplot() + 
  geom_sf(data=county75_bg_work_2022, alpha=1.0, aes(fill=density_grp), color=NA) + 
  scale_fill_manual(values=c("seagreen", "seagreen1","yellow","tan1","red","purple")) +
  theme_classic() + 
  theme(plot.title    = element_text(hjust = 0.5)) +
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  labs(title = "Density of Workers-at-Work, San Francisco County: 2022",
       subtitle = "Census Bureau LEHD/LODES estimates for 2022",
       caption = "Created: CPurvis; Data sources: LEHD/LODES 8.3 (via lehdr)",
       fill = "Workers-at-Work Density Level") +
...
ggsave("sfcounty_workers_at_work_density_2022.png", plot=last_plot(),
       dpi=600,width=8,height=7) 

###################################################################################
# This isn't working. Not sure why. Maybe too many block groups (N=25,585)??????
###################################################################################
ggplot(data=CA_blkgrp_work_2022,
       alpha=1.0, aes(fill=density_grp), color=NA) + 
  scale_fill_manual(values=c("seagreen", "seagreen1","yellow","tan1","red","purple")) +
  theme_void() + 
  labs(title = "Density of Workers-at-Work, California Statewide: 2022",
       subtitle = "Census Bureau LEHD/LODES estimates for 2022",
       caption = "Created: CPurvis; Data sources: LEHD/LODES (via lehdr)",
       fill = "Workers-at-Work Density Level")

ggsave("california_workers_at_work_density_2022.png", plot=last_plot(),
       dpi=600,width=8,height=7)
...
blockgroupx <- CA_blkgrp_work_2022 %>% 
...
write.csv(blockgroupx,   "lehd_California_BlkGrp_Top25_JobDensity_2022.csv")
